...
class HighLevelPolicy(Other):
    ...
    def generate_map(self, episode: Episode.Episode, for_input: bool):
        ...
        if self.include_path_history:
            for point_idx in range(len(episode.path_history) - 1):
                ...
                if point_idx == 0:
                    ax.plot(
                        [point1.x - self.x_min, point2.x - self.x_min],
                        [point1.y - self.y_min, point2.y - self.y_min],
                        **history_plt,
                        zorder=1,
                        label='Path History',
                    )
        ...
        if self.include_state:
            ...
            interval = 40
            ax.set_xticks(
                [i for i in range(0, self.x_n, interval)],
                [int(self.x_min + i * self.resolution) for i in range(0, self.x_n, interval)],
                rotation=90,
            )
            ax.set_yticks(
                [i for i in range(0, self.y_n, interval)],
    ...
    def generate_waypoint(self, episode):
        ...
        if self.include_state:
            ...
            if self.avoid_repeat_waypoints and len(episode.waypoint_history) > 0:
                prompt_str += (
        ...
        if current_goal.x != waypoint.x or current_goal.y != waypoint.y:
            ...
            # otherwise this may result in an infinite loop of generating the same current waypoint
            self.pause(episode)
    ...
    def set_waypoint(self, episode, waypoint):
        ...
        # this also saves the waypoint in episode.waypoint_history
        episode.add_waypoint(waypoint)

        # reset observations since we have a new goal, which will change the previously observed relative distances to goal in observation FIFO queue
